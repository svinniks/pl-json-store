...
WHERE 1=1';
            ...
            FOR v_i IN 1..v_values.COUNT LOOP
                ...
                IF v_values(v_i).name IS NOT NULL 
                   ...
                   AND v_json_stack(v_json_stack.COUNT) = 'O' THEN
                    ...
                    IF REGEXP_LIKE(LOWER(v_values(v_i).name), '^[a-z][a-z0-9_\&]*$') THEN 
                        dbms_lob.append(p_json, '"' || v_values(v_i).name || '":');
                    ELSE
                        dbms_lob.append(p_json, '"' || escape_string(v_values(v_i).name) || '":');
                    END IF;
                ...
                CASE v_values(v_i).type
                    ...
                    WHEN 'S' THEN
                        ...
                        dbms_lob.append(p_json, '"' || escape_string(v_values(v_i).value) || '"');
                        
                    WHEN 'N' THEN
                      
                        dbms_lob.append(p_json, v_values(v_i).value);
    ...
    BEGIN
        ...
        serialize_value(v_value.id, v_json);
